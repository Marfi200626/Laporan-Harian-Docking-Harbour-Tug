...
function signatureCell(report, role) {
  ...
  const signature = isOS ? report.dibuat_oleh_signature : report.diketahui_oleh_signature;
  ...
  if (signature) {
    try {
      const bytes = dataUrlToUint8Array(signature);
      children.push(
        new Paragraph({
          alignment: AlignmentType.CENTER,
          children: [new ImageRun({ data: bytes, transformation: { width: 130, height: 55 } })],
        })
      );
    } catch (e) {
      /* lanjut ke bawah tanpa gambar coretan */
    }
  }
  ...
  if (barcode) {
    try {
      ...
      /* lanjut ke bawah tanpa barcode */
    }
  } else {
    children.push(new Paragraph({ text: "" }), new Paragraph({ text: "" }));
  }

  // Nama, jabatan, dan waktu tanda tangan SELALU ditampilkan
  // (tidak bergantung pada berhasil/tidaknya gambar coretan/barcode)
  children.push(
    new Paragraph({ text: `(${nama || "........................."})`, alignment: AlignmentType.CENTER }),
    new Paragraph({ text: jabatan, alignment: AlignmentType.CENTER })
  );
  ...
  if (timestamp) {
    ...
    children.push(
      new Paragraph({
        alignment: AlignmentType.CENTER,
        children: [new TextRun({ text: `Ditandatangani: ${tanggal}, pukul ${jam} WIB`, italics: true, size: 16 })],
      })
    );
  }
...
export async function exportReportToPdf(report, photos) {
  ...
  async function drawSignatureBox(x, label, nama, jabatan, timestamp, barcode, signature) {
    doc.setFontSize(9);
    doc.setFont("helvetica", "normal");
    ...
    if (signature) {
      try {
        const { w, h } = await getImageDimensions(signature);
        const drawW = 45;
        const drawH = Math.min(20, (h / w) * drawW);
        doc.addImage(signature, "PNG", x, y, drawW, drawH);
        y += drawH + 3;
      } catch (e) {
        /* lanjut tanpa gambar coretan */
      }
    }
    ...
    if (timestamp) {
      ...
      doc.text(`Ditandatangani: ${tanggal}, pukul ${jam} WIB`, x, y);
      doc.setTextColor(0, 0, 0);
    }
  }
  ...
  await drawSignatureBox(
    ...
    report.diketahui_oleh_signature
  );
