...
function drawPdfHeader(doc, logoDataUrl) {
...
function drawPdfFooter(doc, pageNum, totalPages) {
  ...
  const y = pageHeight - FOOTER_TOP_Y_OFFSET + 8;
  ...
  doc.setTextColor(0, 0, 0);
  doc.text(`Hal ${pageNum} dari ${totalPages}`, pageWidth / 2, y, { align: "center" });

  doc.setFont("helvetica", "italic");
  doc.text("FM.TB.414   Rev.000/Jan 2026", pageWidth - MARGIN, y, { align: "right" });
}

function addPdfPage(doc, logoDataUrl) {
  doc.addPage();
  drawPdfHeader(doc, logoDataUrl);
}

function ensureSpace(doc, cursorY, needed, logoDataUrl) {
  const pageHeight = doc.internal.pageSize.getHeight();
  const limit = pageHeight - FOOTER_TOP_Y_OFFSET - 4;
  if (cursorY + needed > limit) {
    addPdfPage(doc, logoDataUrl);
...
export async function exportReportToPdf(report, photos) {
  ...
  autoTable(doc, {
    ...
    didDrawPage: () => drawPdfHeader(doc, logoDataUrl),
  });
  ...
  // ---- Foto Dokumentasi ----
  if (photos.length > 0) {
    cursorY = ensureSpace(doc, cursorY, 10, logoDataUrl);
    ...
    for (let i = 0; i < photos.length; i++) {
      ...
      cursorY = ensureSpace(doc, cursorY, imgHeight + 12, logoDataUrl);
  ...
  // ---- Additional Notes ----
  cursorY = ensureSpace(doc, cursorY, 20, logoDataUrl);
  autoTable(doc, {
    startY: cursorY,
    margin: { left: MARGIN, right: MARGIN, top: HEADER_BOTTOM_Y },
    theme: "grid",
  ...
  // ---- Tanda Tangan ----
  cursorY = ensureSpace(doc, cursorY, 60, logoDataUrl);
  ...
  async function drawSignatureBox(x, label, nama, jabatan, timestamp, barcode) {
    ...
    if (timestamp) {
      doc.setFont("helvetica", "italic");
      doc.setFontSize(7.5);
      doc.text(`Ditandatangani: ${new Date(timestamp).toLocaleString("id-ID")}`, x, y);
    }
  }
